...
const dir = __dirname, out = path.join(dir, 'frames'); fs.mkdirSync(out, { recursive: true });
...
const chrome = spawn(CH, ['--headless=new','--no-sandbox','--disable-gpu-sandbox','--use-gl=angle','--use-angle=swiftshader-webgl','--enable-unsafe-swiftshader',
  '--window-size=1280,720','--remote-debugging-port='+PORT,'--user-data-dir='+path.join(process.env.TMPDIR||'/tmp','chrome-film'),'about:blank'], { stdio: 'ignore' });
...
(async () => {
  ...
  await send('Page.navigate',{url:'file://'+path.join(dir,'film.html')});
